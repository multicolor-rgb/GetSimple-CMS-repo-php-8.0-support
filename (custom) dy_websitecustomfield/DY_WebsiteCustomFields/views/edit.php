<?php
  ...
  if (isset($_GET['undo']) && !isset($_POST['save'])) {    
    if (copy(GSBACKUPSPATH . 'other/' . DY_WEBSITE_CUSTOM_FIELDS_DATA, GSDATAOTHERPATH . DY_WEBSITE_CUSTOM_FIELDS_DATA)) {
    ...
    if (dyWebsiteCustomFieldsSaveData()) {
      $msg = i18n_r('i18n_customfields/SAVE_SUCCESS');
      $success = true;
      if (is_file(GSBACKUPSPATH . 'other/' . DY_WEBSITE_CUSTOM_FIELDS_DATA)) {
        $msg .= ' <a href="load.php?id=' . DY_WEBSITE_CUSTOM_FIELDS . '&amp;undo">' . i18n_r('UNDO') . '</a>';
      }
    } else {
...
<?php } ?>
<h3 class="floated"><?php i18n(DY_WEBSITE_CUSTOM_FIELDS . '/WEBSITECUSTOMFIELDS_TITLE'); ?></h3>
<div class="edit-nav clearfix">
  <a href="load.php?id=<?php echo DY_WEBSITE_CUSTOM_FIELDS; ?>&configure">
    <?php i18n(DY_WEBSITE_CUSTOM_FIELDS . '/WEBSITECUSTOMFIELDS_CONFIGURE') ?>
  </a>
</div>
...
function i18n_customfields_customize_ckeditor($editorvar) { // copied and modified from ckeditor_add_page_link()
  ...
  $data_edit = dyWebsiteCustomFieldsGetData();
  ...
  if (!$defs || count($defs) <= 0) {
    echo '<p>' . i18n_r(DY_WEBSITE_CUSTOM_FIELDS . '/NO_CUSTOMFIELDS') . '</p>';
    return;
  }
  echo '<form class="largeform" method="post">';
  ...
  foreach ($defs as $def) {
    ...
    $label = $def['label'];
    $type = $def['type'];
    $value = htmlspecialchars(isset($data_edit->$key) ? (string) $data_edit->$key : $def['value'], ENT_QUOTES);
    if ($col == 0) {
...
?>
<input type="submit" name="save" value="<?php i18n('BTN_SAVEUPDATES'); ?>" class="submit" id="page_submit" />
</form>