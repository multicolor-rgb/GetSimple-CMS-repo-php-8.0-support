...
add_action('settings-sidebar','createSideMenu',['simple_input_tabs', 'Simple Input Tabs']);
...
function SIT_settings_menu(){
	$form = [['type'=> 'dropdown', 'name'=> 'tab-location', 'label'=> SIT_text('TAB_STYLE'), 'value'=> [SIT_text('JAVA_TABS'), SIT_text('TOP_BUTTONS'), SIT_text('BOTTOM_BUTTONS')]], ['type'=> 'checkbox', 'name'=> 'default-content', 'label'=> SIT_text('DEFAULT_TOGGLE')], ['type'=> 'checkbox', 'name'=> 'error404', 'label'=> SIT_text('ERROR_TOGGLE')], ['type'=> 'checkbox', 'name'=> 'content-top', 'label'=> SIT_text('TOP_CONTENT')], ['type'=> 'checkbox', 'name'=> 'content-bottom', 'label'=> SIT_text('BOTTOM_CONTENT')]];
...
*	SET HOOKS.
*	---------------------------------------------------------------------------------*/
$add_to_list = [['hook' => 'edit-content', 'function' => 'SIT_edit_foot', 'args' => [], 'file' => 'simple_input_tabs.php', 'line' => '26'], ['hook' => 'index-pretemplate', 'function' => 'SIT_generate_data', 'args' => [], 'file' => 'simple_input_tabs.php', 'line' => '26']];
foreach($add_to_list as $insert){
	array_unshift($plugins, $insert);}
function SIT_set_tabs_hook(){
	global $plugins;
	$plugins[] = ['hook' => 'edit-above-content', 'function' => 'SIT_tab_position', 'args' => [], 'file' => 'simple_input_tabs.php', 'line' => '26'];
	$plugins[] = ['hook' => 'index-pretemplate', 'function' => 'SIT_i18n_frontend_fix', 'args' => [], 'file' => 'simple_input_tabs.php', 'line' => '26'];
}	/*-------------------------------------------------------------------------------*/
...
function SIT_template_scan($theme_folder,$theme_file,$function){
	...
	$SIT_WARNING = 0;
	$validated_list = [];//								Establish empty array.
...
//	$the_file = file_get_contents($template_url);//						And scan the file.
	global $FUNCTION_CALLS; $result_list = [];
...
function SIT_save_file(){
	...
	if(isset($_POST['submitted'])){//							Probably pointless to check, but best to be safe.
		...
		if(function_exists("get_magic_quotes_gpc") && get_magic_quotes_gpc()){//							Fix any problems with magic quotes... GRR MAGIC QUOTES! >:[]
