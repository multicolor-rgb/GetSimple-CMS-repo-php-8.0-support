...
function Toolkit_form($data_name,$title,$data_array){
	...
	if(!is_array(reset($data_array))){ $data_array = [$data_array];}// Do we have more than one item?
...
function Toolkit_template($template_file_here=false,$theme_folder_here=false){
	...
	$page_data = $page_data ?: SPT_load_template_locally($template_file_here, $theme_folder_here);
...
function SPT_load_template_locally($template_file_here=false, $theme_folder_here=false){
	...
	$holdtheglobal = [];//		We don't want to change any globals.
...
function Toolkit_page($search){
	...
	$deny = ['content', 'pubdate', 'title', 'url', 'meta', 'metad', 'menu', 'menuorder', 'menustatus', 'template', 'parent', 'content', 'private', 'author', 'cdata']; // Every page has these. So don't search for this string.
...
foreach($SPT_HOOKLIST as $hook=>$loc){//							Prep hooks.
	...
	if(!$loc){
		$add_to_list = ['hook' => $hook, 'function' => $func_name, 'args' => [], 'file' => $file_name, 'line' => '47'];
...
function SPT_common(){
	...
	foreach($SPT_HOOKLIST as $hook=>$loc){
		...
		$file_name = 'small_plugin_toolkit.php';
		if($loc){
			$plugins[] = ['hook' => $hook, 'function' => $func_name, 'args' => [], 'file' => $file_name, 'line' => '47'];}}}
...
function SPT_detect_plugs(){
	...
	$search = ['edit-above-content', 'dev-nav-tab', 'dev-theme-sidebar', 'dev-support-sidebar', 'dev-plugins-sidebar', 'dev-files-sidebar', 'dev-backups-sidebar', 'dev-pages-sidebar', 'register-data'];// Long list of hooks this plugin uses
	$block = ['small_plugin_toolkit', 'caching_functions']; // ignore these.
	$list = [];
